...
function selectf_Callback(hObject, eventdata, handles) %#ok<INUSL,DEFNU>
...
load projectdb; %#ok<LOAD>

k=length(my_database);
disp(k);
for j=1:k
    D(j)=dist(fet',my_database(1,j).feature); %#ok<AGROW>
...
function display_Callback(hObject, eventdata, handles) %#ok<INUSD,DEFNU>
